...
def test_get_float_val():
    ...
    assert textgrid._get_float_val(
        "number = 21.248327456582416") == 21.248327456582416
    assert textgrid._get_float_val(
        "xmax = 4387.9766666666665") == 4387.9766666666665
    assert textgrid._get_float_val(
        "xmin = 5.537098932314087 \n") == 5.537098932314087
...
def test_build_entry_point():
    content = ["points [1]:", "number = 15.140637595485778 ", 'mark = "a"']
    entry = textgrid._build_entry(0, content, tier='testtier')
    assert entry.begin == 15.140637595485778
    assert entry.end == 15.140637595485778
    assert entry.name == "a"
...
def test_build_entry_interval():
    content = [
        "intervals [4]:", "xmin = 5.537098932314087",
        "xmax = 8.711490373278787", 'text = "foo"'
    ]
    entry = textgrid._build_entry(0, content, tier='testtier')
    assert entry.begin == 5.537098932314087
    assert entry.end == 8.711490373278787
...
def test_read_short():
    ...
    assert len(tgrid) == 1
    entry = tgrid[0]
    assert entry.begin == 0
    assert entry.end == 5.537098932314087
    assert entry.name == "z"
